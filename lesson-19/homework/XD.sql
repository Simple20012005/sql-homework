create database F33_homework19
...
create proc GetEmp @percentage int,@department varchar(50)
as begin
UPDATE Employees
...
IF OBJECT_ID('dbo.Tree', 'U') IS NOT NULL
    DROP TABLE dbo.Tree;
...
INSERT INTO dbo.Tree (id, p_id) VALUES (1, NULL);
INSERT INTO dbo.Tree (id, p_id) VALUES (2, 1);
INSERT INTO dbo.Tree (id, p_id) VALUES (3, 1);
INSERT INTO dbo.Tree (id, p_id) VALUES (4, 2);
INSERT INTO dbo.Tree (id, p_id) VALUES (5, 2);
...
CREATE TABLE Confirmations (
    user_id INT,
    time_stamp DATETIME,
    action VARCHAR(20)   -- ENUM o‘rniga VARCHAR ishlatyapmiz
);

-- Signups jadvaliga INSERT
...
-- Confirmations jadvaliga INSERT
...
select s.user_id,round(isnull(n.confirmation_rate,0),2) confirmation_rate from signups s
left join (SELECT user_id ,
    COUNT(CASE WHEN action = 'confirmed' THEN 1 END) * 1.0 /
    COUNT(*) AS confirmation_rate
FROM confirmations
GROUP BY user_id) n on s.user_id=n.user_id
...
as 
begin 
with cte1 as(select p.ProductID,p.ProductName,
s.Quantity,
p.Price,
max(SaleDate) over (partition by productname) as LastSaleDate,
min(SaleDate) over (partition by productname) as FirstSaleDate from Sales s
right join Products p on s.ProductID=p.ProductID
where p.ProductID=@ProductID)
select ProductName,sum(Quantity) as	TotalQuantitySold	,sum(Quantity)*Price as TotalSalesAmount,LastSaleDate,FirstSaleDate from cte1
group by ProductName,Price,LastSaleDate,FirstSaleDate
return
end
