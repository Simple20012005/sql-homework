create database F33_homework13
...
select FIRST_NAME FirstName,len(first_name) NameLength from Employees
...
select id, case
when (isnull(ssn,'')+isnull(passportid,'')+isnull(itin,''))='' 
then 'null' 
else (isnull(ssn,'')+isnull(passportid,'')+isnull(itin,'')) end from person
...
right(FullName,CHARINDEX(' ',REVERSE(FullName))-1) as Lastname from Students
...
from (select *,CONCAT_ws(' ',FIRST_NAME,LAST_NAME) as FL from Employees) sub
where len(fl)-len(REPLACE(FL,'a',''))>=3
...
select e1.DEPARTMENT_ID,count(e1.EMPLOYEE_ID), 1.0*count(e1.EMPLOYEE_ID)/(select count(*) from Employees e3 where e3.DEPARTMENT_ID = e1.DEPARTMENT_ID) TotalEmp from Employees e1
join(select DEPARTMENT_ID,count(EMPLOYEE_ID) Total3  from Employees
...
select 
	p2.JobDescription, 
	p2.SpacemanID as MostExperienced, 
	p3.SpacemanID as LeastExperienced
from 
	(select JobDescription,max(MissionCount) Max, min(MissionCount) Min from Personal
	group by JobDescription) p
JOIN 
	Personal p2
on p.Max = p2.MissionCount
join 
	Personal p3
on p.Min = p3.MissionCount
...
with number as(
select cast(left('tf56sd#%OqH', 1) as varchar(20)) as string, 1 as n 
union all
select cast(substring ('tf56sd#%OqH',n+1,1) as varchar(20)), n + 1 from number
where n < len('tf56sd#%OqH')
),
cte2 as(
select string,
	case
           when ascii(string) between 65 and 90 then 'Uppercase'
		   when ascii(string) between 97 and 122 then 'Lowercase'
		   when ascii(string) between 48 and 57 then 'number'
		   else 'Others'
		   end as class
from number)

select
	(select string_agg(string, '') from cte2 where class = 'Uppercase'),
	(select string_agg(string, '') from cte2 where class = 'Lowercase'),
	(select string_agg(string, '') from cte2 where class = 'number'),
	(select string_agg(string, '') from cte2 where class = 'Others')

--2.Write an SQL query that replaces each row with the sum of its value and the previous rows' value. (Students table)

select *,sum(grade) over(order by  grade) from Students
...
select Equation, REPLACE(Equation, '-', '+-') from Equations

select 
	equation, 
	sum(cast(value as int))
from Equations 
cross apply string_split(REPLACE(Equation, '-', '+-'),'+')
group by Equation
--4.Given the following dataset, find the students that share the same birthday.(Student Table)

select Birthday,string_agg(StudentName,',') from Student
group by Birthday
...
select pa as PlayerA,pb AS PlayerB,sum(score) TotalScore from (select *,case 
         when PlayerA>PlayerB then PlayerA
		 else PlayerB end as pa,
		 case 
		 when PlayerB>PlayerA then PlayerA
		 else PlayerB end as pb
from PlayerScores)sub
group by pa,pb
