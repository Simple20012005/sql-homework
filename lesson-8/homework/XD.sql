create database F33_homework_lesson8

use F33_homework_lesson8
...
select Category,count(ProductName)
from Products
group by Category
...
Select  year,[Bektemir], [Chilonzor], [Yakkasaroy], [Yashnobod], [Mirobod]
into Population_Each_City
from (SELECT  District_Name, Population, Year
    FROM City_Population
) AS SourceTable
pivot 
(sum(population) for District_Name in ([Bektemir], [Chilonzor], [Yakkasaroy], [Yashnobod], [Mirobod])) as pivottable
...
from (select district_id,district_name,[2012],[2013] from Population_Each_Year)
as soursetable
unpivot(
...
from (select year,[Bektemir],[Chilonzor],[Yakkasaroy],[Yashnobod],[Mirobod] from Population_Each_City)
as SourseTable
unpivot 
(population for District_Name in ([Bektemir],[Chilonzor],[Yakkasaroy],[Yashnobod],[Mirobod])) as unpivottable
