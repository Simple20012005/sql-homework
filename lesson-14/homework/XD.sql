create database F33_homework14
...
select  left(name,charindex(',',name)-1) as Name,right(name,charindex(',',reverse(name))-1) as Surname from TestMultipleColumns
...
join Employee e1 on e1.Id=e.ManagerId and e1.Salary<e.Salary
...
where DATEDIFF(year,hire_date,getdate()) between 11 and 15
...
select  replace(TRANSLATE('rtcfvty34redt','0123456789','          '),' ','') as Others,
replace(TRANSLATE('rtcfvty34redt',
replace(TRANSLATE('rtcfvty34redt','0123456789','          '),' ',''),
REPLICATE(' ', len(replace(TRANSLATE('rtcfvty34redt','0123456789','          '),' ','')))),' ','') as Int


--2.write a SQL query to find all dates' Ids with higher temperature compared to its previous (yesterday's) dates.(weather)

with cte as (select *,lag(Temperature) over(order by recordDate) as Previous from weather)
select id,RecordDate,Temperature from cte
...
select p1.id,p2.code from p2 join
p1 on p2.id=p1.id and p1.code=0
union all
select p1.id,p1.code from p2 join
p1 on p2.id=p1.id and p1.code<>0
...
SELECT 
    ...
    CASE 
        ...
        WHEN DATEDIFF(YEAR, HIRE_DATE, GETDATE()) BETWEEN 1 AND 5 
            THEN 'Junior'
        WHEN DATEDIFF(YEAR, HIRE_DATE, GETDATE()) BETWEEN 6 AND 10 
            THEN 'Mid-Level'
        WHEN DATEDIFF(YEAR, HIRE_DATE, GETDATE()) BETWEEN 11 AND 20 
            THEN 'Senior'
        ELSE 'Veteran'
...
SELECT 
    Vals,
    CAST(LEFT(Vals, PATINDEX('%[^0-9]%', Vals + 'a') - 1) AS INT) AS StartInteger
FROM GetIntegers
WHERE Vals LIKE '[0-9]%';
...
SELECT 
    ...
    Vals,
    CONCAT(
        PARSENAME(REPLACE(Vals, ',', '.'), 2), ',',   -- ikkinchi element
        PARSENAME(REPLACE(Vals, ',', '.'), 3), ',',   -- birinchi element
        STUFF(Vals, 1, CHARINDEX(',', Vals + ',') 
              + CHARINDEX(',', STUFF(Vals, 1, CHARINDEX(',', Vals), '')), '')  -- qolganlari
    ) AS SwappedVals
FROM MultipleVals;
...
WITH area_weekly_sales AS (
    SELECT
        Area,
        FinancialYear,
        FinancialWeek,
        SUM(COALESCE(SalesLocal,0) + COALESCE(SalesRemote,0)) AS TotalAreaSales
...
week_totals AS (
    SELECT
        FinancialYear,
        FinancialWeek,
        SUM(TotalAreaSales) AS TotalWeekSales
    FROM area_weekly_sales
...
SELECT
    ...
    a.Area,
    a.TotalAreaSales,
    CAST(a.TotalAreaSales * 100.0 / w.TotalWeekSales AS DECIMAL(5,2)) AS PercentageOfWeek
FROM area_weekly_sales a
JOIN week_totals w
  ON a.FinancialYear = w.FinancialYear
 AND a.FinancialWeek = w.FinancialWeek
ORDER BY a.FinancialYear, a.FinancialWeek, a.Area;
