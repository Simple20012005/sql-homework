create database F33_hoomework23
...
with cte as (select  count(distinct id)  as Distinct_Ids ,
rID
from MyTabel
group by rID)
select *,(select sum(maxvals) as TotalOfMaxVals from (select rid,max(vals)  as Maxvals from MyTabel
group by id,rid) sub   where sub.rID = cte.rID ) as TotalOfMaxVals from cte
...
with cte as(select *,rank() over(partition by id order by vals desc) as Rank from TestMaximum)
...
with cte as(select id,DetailedNumber,max(Vals) as max
from SumOfMax
group by id,DetailedNumber )
select id,sum(max) SumofMax from cte 
group by id
...
select count(SaleID) as SaleCount from Sales

--10.What is the highest number of units sold in a single transaction?

select max(QuantitySold) as MaxUnits from Sales

--11.How many products were sold in each category?

select Category,sum(QuantitySold) as SaleCount from Sales
group by Category
...
select top 1 product,max(QuantitySold*UnitPrice) as MaxTotalRevenue from Sales
group by Product
...
select Category,sum(QuantitySold*UnitPrice)/(select sum(QuantitySold*UnitPrice) from Sales)*100 as Percentage from Sales
group by Category
...
select s.SaleID,s.Product,s.Category,s.QuantitySold,s.UnitPrice,s.SaleDate,s.Region,c.CustomerName from Sales s
join Customers c on c.CustomerID=s.CustomerID

--18.List customers who have not made any purchases

select c.CustomerName from Sales s
right join Customers c on c.CustomerID=s.CustomerID
where s.SaleID is null

--19.Compute total revenue generated from each customer

select s.CustomerID,c.CustomerName,sum(QuantitySold*UnitPrice) as TotalRevenue from Sales s
join Customers c on c.CustomerID=s.CustomerID
...
select top 1 s.CustomerID,c.CustomerName,sum(QuantitySold*UnitPrice) as TotalRevenue from Sales s
join Customers c on c.CustomerID=s.CustomerID
...
select s.CustomerID,c.CustomerName,sum(QuantitySold) as Totalsales from Sales s
join Customers c on c.CustomerID=s.CustomerID
group by c.CustomerName,s.CustomerID
...
left join Sales s on s.Product=p.ProductName
where s.SaleID is not null

--23.Find the most expensive product in the Products table

select top 1 ProductName,SellingPrice as Expensive from Products
...
with cte as (select *,avg(SellingPrice) over(partition by category) as Avg from Products)
select ProductID,ProductName,Category,CostPrice,SellingPrice from cte 
where SellingPrice>Avg
