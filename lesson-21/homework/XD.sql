create database F33_homework21
...
select *,ROW_NUMBER() over(partition by saledate order by saleamount) from ProductSales
...
select ProductName,sum(Quantity) TotalQuantity,DENSE_RANK() over(order by sum(Quantity)) Rank from ProductSales
group by ProductName;

--3.Write a query to identify the top sale for each customer based on the SaleAmount.

with Xd as(select CustomerID,SaleAmount,DENSE_RANK() over(partition by customerId order by saleamount) as Rank from ProductSales)
select * from xd
where rank=1;

--4.Write a query to display each sale's amount along with the next sale amount in the order of SaleDate.

select SaleAmount,lead(SaleAmount) over(order by saledate ) NextDaysale from ProductSales

--5.Write a query to display each sale's amount along with the previous sale amount in the order of SaleDate.

select SaleAmount,lag(SaleAmount) over(order by saledate ) NextDaysale from ProductSales;

--6.Write a query to identify sales amounts that are greater than the previous sale's amount

with XD as(select SaleAmount,lag(SaleAmount) over(order by saledate ) NextDaysale from ProductSales
)
select * from Xd
where SaleAmount>NextDaysale

--7.Write a query to calculate the difference in sale amount from the previous sale for every product

select saleid,ProductName,abs(SaleAmount-isnull(lag(SaleAmount) over(partition by Productname order by saledate ),0)) Difference from ProductSales;

--8.Write a query to compare the current sale amount with the next sale amount in terms of percentage change.

select SaleID,ProductName,SaleAmount, lead(SaleAmount) over(order by saledate ) Nextdaysale,
CASE 
    WHEN SaleAmount = 0 THEN NULL
    ELSE 
      ((LEAD(SaleAmount) OVER(ORDER BY SaleDate) /SaleAmount*100-100))
  END AS PercentChange  from ProductSales
...
lag(SaleAmount) over(partition by productname order by saledate ) NextDaysale,
SaleAmount/lag(SaleAmount) over(partition by productname order by saledate ) Ratio from ProductSales;


--10.Write a query to calculate the difference in sale amount from the very first sale of that product.

select *,rank() over(partition by productname order by saledate ) from ProductSales
...
select SaleID,ProductName,SaleAmount,nextdaysale from 
(select SaleID,ProductName,SaleAmount,isnull(lag(SaleAmount) over(partition by productname order by saledate ),0) NextDaysale from ProductSales) sub
where SaleAmount>NextDaysale ;
...
select saledate,sum(saleamount) over(order by saledate) as ClosingBalance from ProductSales

--13.Write a query to calculate the moving average of sales amounts over the last 3 sales.

SELECT *,avg(saleamount) over(order by saledate rows between 2 preceding and current row) as last3sales from ProductSales

--14.Write a query to show the difference between each sale amount and the average sale amount.


Select SaleAmount-(avg(SaleAmount) over ()) as Difference  from ProductSales
...
with cte as(select *,RANK() over (order by salary) samesalary from Employees1)
select * from cte 
where samesalary in(select samesalary from cte
group by samesalary
having count(*)>1);

--16.Identify the Top 2 Highest Salaries in Each Department

with cte as(select *,DENSE_RANK() over (partition by department order by salary) as Rank from Employees1)
select EmployeeID,Name,Department,Salary,HireDate from cte
where Rank<=2

--17.Find the Lowest-Paid Employee in Each Department

with cte as(select *,min(salary) over (partition by department) as minsalary from Employees1)
select EmployeeID,Name,Department,Salary,HireDate from cte
where Salary=minsalary

--18.Calculate the Running Total of Salaries in Each Department

select *,sum(salary) over (partition by department order by employeeid) as sumsalary from Employees1;
...
from Employees1)
select Department,TotalSalary from cte
where rank=1
...
rank() over (partition by department order by salary ) as Rank 
from Employees1)
select Department,AvgSalary from cte
where rank=1

--21.Find the Difference Between an Employee’s Salary and Their Department’s Average

select *,Salary-(avg(salary) over(partition by department )) as Difference from Employees1
...
with cte as (select top 3 * from Employees1
order by HireDate desc)
select sum(salary) as Totalsalary from cte
